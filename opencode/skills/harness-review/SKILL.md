...
|--------|--------|-----------------|
| **major** | 仕様正本に書かれた権限/契約/API と実装が明確に矛盾している、または仕様正本が必要な task なのに `spec_path` も `spec_skip_reason` もない | 1 件でも → REQUEST_CHANGES |
| **minor** | spec link の書き方が弱いが、実装の正しさは contract / test / existing docs で確認できる | verdict に影響しない |
