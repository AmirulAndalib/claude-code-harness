...
| R05 | Bash | `rm -rf` / `rm --recursive` | ask（設定で warn 可: 綴りが root 配下 / 自セッション scratch / 相対なら approve + warn + `.claude/state/destructive-delete.jsonl` 記録。root 外・`..`・未解決 `$VAR`・glob・`.` は warn でも ask） | workMode / `destructive_delete` |
