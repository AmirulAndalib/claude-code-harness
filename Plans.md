...
注記 (2026-08-02 訂正): PR #282 は squash merge されたため、worktree 側で作成された commit (旧記載 `7702ca45` / `476ea403`) は main から到達不可。台帳は main 上の実体 `1f085a35` を指すよう訂正した。
...
dependabot ブランチ上では作業しない。あちらの lockfile は `typescript ^6.0.3` の時点で切られており、main は既に `^7.0.2` へ進んでいる (mergeStateStatus: BEHIND)。main から新規ブランチを切って lockfile を再生成し、main へ入った時点で #272 を自動 close させる。
