...
**Purpose**: Claude (Opus 5 / CC CLI) / Codex (gpt-5.6) / Grok (grok-cli v1.1.7) / Cursor CLI の 2026-08 時点仕様を 5 並列調査した結果のうち、確証が取れて採用価値のあるものを task 化する。調査 evidence は各 task の内容に記載。モデル pin の変更はすべて operator 判断 (2026-07-25 Model Routing 裁定が正本) であり、AI が自動適用しない。
...
| 133.4 | `[lane:gate]` repair loop の状態外部化。現状 `skills/harness-work/references/review-loop.md` の修正ループは同一会話内で反復し、iteration 数の管理も会話内 (context rot + 上限が自己申告)。ループ状態 (iteration / verdict / findings) を `.claude/state/repair-loop/<task>.json` へ外部化し、上限判定を機械化する。3 巡目以降は fresh-context Worker で修正する選択肢も検討 | (a) 状態ファイル schema + 書き込み配線, (b) MAX_REVIEWS 超過が状態ファイル基準で判定される, (c) mirrors sync PASS | - | cc:TODO |
| 133.5 | `[lane:fast]` blind judge (rubric を見せない第二審) を harness-review の opt-in として追加。対象は主観品質が支配する外部向け成果物 (UI copy / docs / cognitive-load HTML) のみ。fresh reviewer に成果物 + 読者/目的 1 行だけを渡し、rubric verdict との大きな乖離を独立 finding として報告する (auto-REQUEST_CHANGES にはしない) | (a) SKILL.md に opt-in 手順と対象条件, (b) rubric 併走時の出力契約 (乖離 finding の形式), (c) mirrors sync PASS | - | cc:TODO |
| 133.6 | `[lane:fast]` CC CLI 新機能の一次ソース検証。調査 agent が WebFetch 要約経由で報告した (i) subagent の nested spawn depth / concurrency env、(ii) sandbox credential-masking / strictAllowlist は**バージョン番号未確証**。raw CHANGELOG を直接読み、確証が取れたものだけ `docs/CLAUDE-feature-table.md` へ追記し、(ii) が実在すれば `docs/sandbox-allowlist-recipe.md` / `defense-layer-blast-radius.md` の denyRead 回避策を新機構で置き換える提案を出す | (a) 一次ソース (raw changelog) の引用つき検証記録, (b) 確証分のみ feature-table 追記, (c) `bash scripts/ci/check-consistency.sh` PASS | - | cc:TODO |
