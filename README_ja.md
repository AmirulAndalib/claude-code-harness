...
| `/harness-work` | 承認された作業を 1 件実装する。必要な作業ではテストを先に書く | 作業がそう指定していればテストを先に書く |
...
同じリポジトリで 3 つのエージェントを開くと、通常は互いを見ないまま作業する。これは小さな非効率ではない。CooperBench の測定では、同一ファイルを 2 エージェントが編集すると成功率は単独作業の約半分に落ち、失敗の 63% が「相手が何を変えたか」についての誤った思い込みから来ていた。

Harness はローカルのセッション間に名簿と連絡路を持つ。

| 部品 | 何をするか |
|---|---|
| 名簿 | `bin/harness session list` がこのマシン上の稼働セッションを一覧する。保存先を `git --git-common-dir` から解決するため、別 worktree のセッションも同じ名簿に載る。各行には送信に必要な `team` と `agent` が入る |
...
パイプラインそのものが成果物であり、何を流すかは利用者が決める。送信は既定では素通りする。`[livemsg] verification = "on"` にすると関所が加わり、メッセージ中の事実の主張 (言及したファイルが実在するか、commit が解決するか、「変更なし」の主張が `git status` と一致するか) を検査して、虚偽なら配送せず理由を送信側に返す。`off` の間は送信経路が関所を呼びもしない。

これはローカル完結で、harness-mem に依存しない。harness-mem を併用している場合も、その名簿エントリは触らずに保持する。
