...
なお複数行にまたがる二重引用符文字列 (`python3 -c "..."` のように `"` の開始行と終了行が異なる場合) は、引用符状態を論理行単位でリセットする検出器の実装上の限界により、引き続き検出対象外です。目視レビューで実例を 3 件発見し herestring 化しましたが、検出器自体はこの形を機械検出できません。

| 項目 | 変更前 | 変更後 |
|---|---|---|
| producer の対象 | `printf` / `echo` / `cat` の 3 種のみ | 種類を問わない |
| 検出漏れ | jq / find / git / grep / head / シェル関数呼び出しなど | なし (複数行にまたがる二重引用符文字列を除く) |
